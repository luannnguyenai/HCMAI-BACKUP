...
- **`aic2026.eval.demo`** (SS 6) - the live demo surface: a curated `CANNED_EXAMPLES` set (12 examples) keyed by `NoiseMode`, weighted toward `mixed_ocr` (5 of 12 - the realistic-worst-case mode C1 wins on) and covering every failure family C1 was built to attack: 2x `drop_all` (short placename + long sentence with a foreign token), 2x `char_confuse` (address + sports score), `word_merge`, `homophone_swap` (the `Mỹ Tâm` -> `Mỹ Tấm` ASR case), and a clean-sanity control kept last. Each non-control example pins a `noise_seed` chosen to produce dramatic-but-readable corruption (so the demo isn't seed-roulette). A side-by-side block formatter marks the gold target with `[TRÚNG]`, a `run_canned` batched runner emits an **honest 4-way tally** judged on a top-`useful_k` (default 10) usefulness lens - `clean_win` (C1 surfaces the gold in top-`useful_k`, baselines don't), `tie` (both usable), `graceful` ("C1 TRỤ TỐT HƠN" - neither usable but C1 far closer; this is the honest label for the `#50` vs `#1370` case so a non-usable result is never sold as a win), and `loss` - and a `run_interactive` REPL handles audience queries. A `tune_seeds` mode sweeps `noise_seed` per example and recommends the seed giving the cleanest C1 win (used to re-tune the canned set when the head changes). CPU-testable via injected `FakeRetriever`; GPU-runs via the same three retrievers the ship-gate uses, so what the demo shows and what the eval measures cannot diverge.
...
- **AC10** (SS 6, live demo): `CannedExample.make_noised` is deterministic and matches the training-time `noise()` distribution; the canned set is >= 10 examples (currently 12), covers `DROP_ALL`, `CHAR_CONFUSE`, `MIXED_OCR`, `WORD_MERGE`, `HOMOPHONE_SWAP`, and exactly one clean-sanity example kept last, with `mixed_ocr` the most-represented mode (>= 3), unique ids, and every non-control example producing visible noise (its noised form differs from the clean target). `format_example_block` marks the gold target with `[TRÚNG]` at the correct rank, reports gold position when it falls outside top-k, and renders an "(không có — đối chứng)" line for clean-sanity examples. `_classify_verdict` returns the honest 4-way verdict (`clean_win` / `tie` / `graceful` / `loss`) under a top-`useful_k` lens, with the boundary (`rank == useful_k` is usable) verified; `run_canned` returns the correct `{clean_wins, ties, graceful, losses}` tally for hand-built `FakeRetriever` score matrices and prints a Vietnamese summary; the doc index combines `doc_pool` with canned targets (deduped, insertion-order preserved) so the gold answer is always indexable. `_recommend_seed` prefers a clean-win seed with the widest separation, falling back to the best graceful seed; `tune_seeds` returns a recommended seed per non-control example (in range), skipping the control. `run_interactive` is EOF-safe (immediate exit on empty query; clean exit on EOF mid-session) and counts queries executed. Verified in `tests/unit/test_eval_demo_AC1.py` (25 tests; CPU-only via `FakeRetriever`, no torch import).
...
- **Q2**: noise-schedule realism. The `Beta(2, 5)` drop rate is an assumption. proposal 04 part 6 (risk row) says validate it against a ~200-sample slice of real PhoWhisper / PaddleOCR output and retune. That real slice does not exist until we have ASR/OCR over the corpus (post-June-25). Until then, sanity-check the noise visually against a handful of known PhoWhisper error examples and record the assumption in `train_meta.json`. **v2 update:** OCR-character noise modes (`space_split`, `char_confuse`, `mixed_ocr`) were added in 2026-06-01 to cover failure modes the v1 schedule missed: PaddleOCR over-segmentation (`q u ả t á o`) and visual confusables (`5<->S`, `1<->l`, etc.). **v3 Tier A update:** following the Hoang & Aw (2012, EACL workshop, "Spell Checking in Vietnamese OCR-scanned Texts") 3-class taxonomy survey, three more modes were added on the same day to close the remaining high-impact gaps: `word_merge` (Class 3 under-segmentation, the inverse of `space_split` that v2 missed), `homophone_swap` (Class 2 real-syllable substitution -- the dominant PhoWhisper failure where the same base syllable confuses across its 5-6 tonal homophones; mark-anchored `tone_swap` cannot reach this), and `case_noise` (Class 3 case shift, preserved by spell-check). Same Beta(2,5) per-string severity prior; final calibration against real OCR output still post-June-25. **Calibration harness shipped (2026-06-02):** `aic2026.train.calibrate` + `bin/train c1-calibrate` profile surface statistics (length, diacritic density, digit ratio, casing, and the single-char-token ratio that tells OCR over-segmentation) of {real queries, our anchors, our synthetic per-mode noise, real OCR output} and emit advisory flags (anchor-vs-query length mismatch; `mixed_ocr` over/under-fragmentation vs real OCR; digit-density mismatch). It calibrates by **distribution matching of surface statistics**, not a true error rate (we lack aligned clean/noisy pairs). The real text now exists *pre*-June-25 via the **AIC2025 proxy corpus** ([research-note 07](../research-notes/07-aic2025-proxy-corpus.md)): the `query/` set is the clean-query distribution, and OCR/ASR over its keyframes is the real noisy distribution. Q2 moves from "blocked on June-25" to "runnable now on the proxy corpus". **First calibration run (2026-06-02, AIC2025 proxy):** OCR over 916 text-bearing keyframes (EasyOCR `vi` -- PaddleOCR PP-OCRv5 was unusable on the box: PaddlePaddle CPU PIR-executor bug) vs our synthetic noise over 5000 Wikipedia anchors surfaced a **3-axis mis-calibration of `mixed_ocr` vs real OCR**: (1) **diacritic drop too mild** -- real OCR diacritic-density **0.062** vs clean 0.29 (real strips ~80%) but our `mixed_ocr` only reaches 0.23; (2) **uppercase under-modelled** -- real scene/TV OCR is **40% uppercase** (`upper_ratio` 0.40) vs our `mixed_ocr` 0.04 (`case_noise` alone hits 0.21); (3) **over-fragmentation** -- our `space_split`/`mixed_ocr` single-char-token ratio **0.52** vs real OCR **0.084** (real OCR keeps words intact at token level). **v4 retune direction:** in `mixed_ocr`, raise the diacritic-drop weight, fold in heavier `case_noise`, and cut the `space_split` probability. Caveats: EasyOCR box-joins text so its low fragmentation is partly an engine artifact (PaddleOCR may fragment more -- re-measure when PP-OCRv5 runs); the real-query diacritic figure (0.10) is diluted by the xlsx's English `Trans` column + IDs and is not a true signal. Full numbers in [research-note 07](../research-notes/07-aic2025-proxy-corpus.md) §4.2.
...
| 2026-06-01 | implementer | **Live demo surface (SS 6).** Added `aic2026.eval.demo` (`CannedExample`, `CANNED_EXAMPLES`, `format_example_block`, `run_canned`, `run_interactive`), `bin/train c1-demo {canned\|interactive\|both}`, `infra/remote/c1_demo.sh` runner, and AC10 (initially 12 CPU-only tests via `FakeRetriever`). Canned set curated to exhibit one strength per failure family C1 was built to attack. The demo wires the *same* three retrievers as the ship-gate (`MaxSimRetriever(head)` vs `MaxSimRetriever(None)` vs `DenseRetriever(BgeM3DenseEmbedder)`), so what the demo shows and what the eval measures cannot diverge. Vietnamese labels throughout for external presentation. Restores `head.pt` + `pairs.parquet` from R2 `c1-baseline/<sha>/` exactly like `c1_eval.sh`; tested with the v3 head. |
| 2026-06-01 | implementer | **Live demo: honest verdict + 12 examples + seed tuner (SS 6).** First demo run (5 examples, n_docs=300) saturated to mostly ties; expanded the canned set to **12 examples weighted toward `mixed_ocr` (5 of 12)** and bumped the default index to `n_docs=2000` (the user's 2000-doc run showed baselines dropping the gold to rank #12-#1661 while C1 held #1-#50). Critically, the first 2000-doc run exposed a **misleading verdict**: a `mixed_ocr` case where C1 ranked the gold #50 and baselines #1370/#1661 was reported a flat "win" - but #50 is not a *usable* result. Replaced the binary win/tie/loss with an honest 4-way verdict under a top-`useful_k` (default 10) lens: `clean_win` (C1 usable, baselines not), `tie` (both usable), `graceful` ("C1 TRỤ TỐT HƠN" - neither usable, C1 far closer), `loss`. Added `tune_seeds` (`--mode tune`) to sweep `noise_seed` per example and recommend the seed giving the cleanest win, so the canned set can be re-tuned on the box when the head changes. `bin/train c1-demo` gains `--mode tune`, `--useful-k`, `--sweep-n`. AC10 expanded to 25 CPU-only tests. |
| 2026-06-02 | implementer | **Q2 first calibration result (AIC2025 proxy, n=916 OCR frames).** Ran `ocr_sample.py` (EasyOCR `vi`; PaddleOCR PP-OCRv5 blocked by a PaddlePaddle CPU PIR bug on the box) over sampled keyframes -> `c1-calibrate`. Real OCR vs our `mixed_ocr` is mis-calibrated on 3 axes: diacritic-drop too mild (real 0.062 vs our 0.23), uppercase under-modelled (real **0.40** vs our 0.04), and over-fragmentation (our single-char-token 0.52 vs real 0.084). **v4 mixed_ocr**: heavier diacritic drop + fold in `case_noise` + cut `space_split` rate. Caveats: EasyOCR box-joins (fragmentation is partly an engine artifact -- re-check with PP-OCRv5); query-diacritic figure diluted by the xlsx English column. Tooling: `infra/remote/ocr_sample.py`. Full numbers in research-note 07 §4.2. |
| 2026-06-02 | implementer | **Q2 noise-realism calibration harness (against the AIC2025 proxy corpus).** Shipped `aic2026.train.calibrate` (`TextStats`, `profile_text`, `profile_synthetic_noise`, `compare`, `load_strings`) + `bin/train c1-calibrate` + 9 CPU-only tests (`test_calibrate_AC1.py`). Calibrates the noise schedule by distribution-matching surface statistics of real queries / our anchors / our synthetic per-mode noise / real OCR output, flagging which knob to revisit (anchor-vs-query length, `mixed_ocr` fragmentation via single-char-token ratio, digit density). Unblocked *pre*-June-25 by the AIC2025 proxy corpus ([research-note 07](../research-notes/07-aic2025-proxy-corpus.md)); the actual retune is pending the on-box run. Does not compute a true error rate (no aligned clean/noisy pairs) - documented limitation. |
| 2026-06-01 | implementer | **Demo seeds tuned on the H200 (5/12 clean wins).** Ran `c1-demo --mode tune` (sweep 0-15, useful_k=10) against the v3 head at n_docs=2000. Confirmed the honest pattern: the **6 single-mode examples** (drop_all x2, char_confuse x2, word_merge, homophone_swap) **tie on every seed** - BGE-M3 already handles single-axis noise at sentence length, so C1 neither helps nor hurts (the no-regression story). Only **combined `mixed_ocr` noise** breaks the baselines, and every `mixed_ocr` example has a clean-win seed. Baked the recommended seeds: `ex3=7` (C1 #1 vs #930/#1453), `ex6=0` (C1 #5 vs #67/#147), `ex7=7` (C1 #5 vs #1192/#1536), `ex8=6` (C1 #1 vs #308/#323), `ex9=13` (C1 #4 vs #864/#1035). Updated the 3 explanation strings that quoted stale corruptions (`75 -> 7 S`, `Long Biên -> Lohq 8ich`, `1546 -> l s 4 6`). Expected canned tally: **5 clean wins / 7 ties / 0 graceful / 0 losses** - 5 decisive `mixed_ocr` wins where baselines drop the gold to rank #67-#1536, the rest honest ties incl. the control. Seeds are stable for the banked v3 head + (n_docs=2000, seed=0); re-run `--mode tune` if the head is retrained. |
