...
- **Q2**: Qwen3-VL-Embedding-2B `transformers` API + min version + `trust_remote_code` - verify against the HF card on the box; if the integration is blocked, fall back to the model's `vLLM` serving path or defer 2B and record the 8B-offline-only stance.
- **Q3**: provided `clip-features-32` layout (per-video `.npy`? id keying?) - inspect on the box; determines whether the provided-CLIP lane is includable.
- **Q4**: INT4 footprint method - `bitsandbytes` 4-bit load vs a documented param-count estimate; pick whichever runs cleanly on the box.
