# Research Note 06 — AIC2026 dataset shape (partial team-channel intel)

> Captures what we know about the **official AIC2026 dataset** format ahead of the June 25, 2026 release. Source: team-channel discussion on 2026-05-28 with a teammate ("Hoàng") who has prior-edition context. This is **provisional**; supersede with organiser-issued documentation when it lands.
...
| **CLIP embeddings** | yes, pre-computed | **organisers' embedding model is reportedly weak** (teammate: "hàng c?a BTC xài model cùi nên không trông ch? gì") — treat as baseline lane only |
...
> "Metadata s? có url youtube -> yt-dlp trích xu?t transcript (vi) không có thì fallback dùng model Whisper. (Còn máy x?n process cái vèo thì thôi kh?i ch?n :))"
...
1. **PhoWhisper licence risk drops** (master strategy §10 item 2). If yt-dlp covers most of the corpus, our hard dependency on a CC-BY-NC-SA model shrinks to "fallback when captions absent". In the limit, we may not need PhoWhisper at all - depends on the empirical fraction.
...
The teammate confirms what we already assumed: the provided CLIP is "model cùi" — weak. Our [ADR-0007 Edge contribution C2](../adr/ADR-0007-original-contributions-c1-c2-c4.md) and the three-encoder ensemble (SigLIP-2 + Meta CLIP 2 + InternVideo2-1B) are precisely what beats it.
...
This adds one ranked list to the per-task-type learned fusion in C2 — measurable cost ~hours of indexing storage; measurable gain a small lift on the slice of queries where the organisers' model happens to find a near-match.
...
- **Elasticsearch** (SPEC-0007): a `description` text field, Vietnamese-aware analyser (NOT asciifolding — same trap as [research-note 05 §4.1](05-baseline-2025-analysis.md)).
...
| Master strategy §10 | Before | After |
|---|---|---|
...
| Q7 (dataset preview) | "open" | "partial team-channel intel captured in research-note 06; full schema awaits June 25 release" |
...
- **Q-DS-3**: How dense is the keyframe sampling - one per second? One per shot? Uniform N per video?
...
All Q-DS items go on the agenda for the post-2025-baseline-author interview ([`docs/permissions/2025-baseline-reuse.md`](../permissions/2025-baseline-reuse.md) §4) **and** on the post-June-25 dataset-shake-down checklist.

## 5. References

- Source: team-channel discussion 2026-05-28; teammate "Hoàng" surfacing recommendations and dataset shape.
...
- TransVLM: search-pending; reference in teammate's message implies a vision-language-model-based shot detector — needs verification.
- yt-dlp: <https://github.com/yt-dlp/yt-dlp>
- PhoWhisper licence (CC-BY-NC-SA-4.0): <https://huggingface.co/vinai/PhoWhisper-large> - see also [strategy §10 item 2](../strategy/00-master-strategy.md).
